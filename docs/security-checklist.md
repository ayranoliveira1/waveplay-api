...
| 19.8 | **Delete de plano com subscriptions vinculadas** | Remover plano quebra integridade referencial e deixa registros orfaos em subscriptions (inclusive historicas) | `DELETE /admin/plans/:id` so executa quando `countSubscriptionsByPlanId === 0` (gate estrito: bloqueia tambem subscriptions canceladas/expiradas, preservando historico). Caso contrario retorna 409 `PlanHasSubscriptionsError`. Frontend tambem filtra opcao "Excluir" baseado em `usersCount` retornado pelo list | Alta |
...
| 19.15 | **Credenciais R2 expostas** | Senhas/secrets do Cloudflare R2 vazadas em logs ou no repositorio | `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`, `R2_PUBLIC_URL` validadas via Zod no boot (`envSchema`). Nunca logadas. `.env` no `.gitignore`. Token R2 com permissao limitada ao bucket especifico (`waveplay`), sem acesso a outros buckets da conta | Critica |
| 19.16 | **Upload de arquivo arbitrario via presigned URL** | Atacante captura presigned URL e sobe arquivo malicioso (executavel, .exe disfarcado de .apk, payload XSS) | URL pre-assinada valida apenas `Content-Type: application/vnd.android.package-archive` e expira em 5 minutos. Apenas admin pode gerar (`@Admin()` decorator + `AdminGuard`). Nome do storageKey segue pattern fixo `apks/{version}.apk` evitando path traversal | Alta |
| 19.17 | **Rate limit em endpoint publico** | Apps mobile bootando simultaneamente sobrecarregam `GET /app/version`, ou atacante abusa do endpoint para enumerar/DoS | `@Throttle({ default: { limit: 60, ttl: 60_000 } })` no `GetCurrentAppVersionController`. 60 req/min por IP e suficiente para boot do app. Throttle global do app continua valendo (300 req/min) | Media |
| 19.18 | **URL publica do APK acessivel a qualquer um** | Bucket R2 publico — qualquer pessoa com link baixa o APK | Aceitavel: distribuicao publica intencional (objetivo da Task 31). Se virar problema, migrar para presigned download URL (ex: validade 15min + assinada pelo backend). Storage key nao e adivinhavel se nao souber o numero da versao publicado | Baixa |
| 19.19 | **Race em set-current entre admins** | Dois admins promovem versoes diferentes simultaneamente, deixando duas com `is_current = true` | `setCurrent(id)` no PrismaRepo usa `prisma.$transaction([updateMany, update])` — atomico. Combinado com partial unique index `WHERE is_current = true` no banco, segunda transacao falha por constraint violation se conflitar | Media |
| 19.20 | **Deletar versao com isCurrent = true deixa app sem versao publica** | Admin deleta a versao current acidentalmente; `GET /app/version` passa a retornar 404 | `DeleteAppVersionUseCase` rejeita com 409 `CannotDeleteCurrentVersionError` quando `version.isCurrent === true`. Admin precisa promover outra versao primeiro | Alta |
