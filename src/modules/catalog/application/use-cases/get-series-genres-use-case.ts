import { Injectable } from '@nestjs/common'

import type { Either } from '@/core/either'
import { right } from '@/core/either'
import { CatalogCacheService } from '../../infra/catalog-cache.service'
import type { TMDBGenre } from '../../domain/ports/catalog-provider.port'

type GetSeriesGenresUseCaseResponse = Either<never, { genres: TMDBGenre[] }>

@Injectable()
export class GetSeriesGenresUseCase {
  constructor(private catalogCache: CatalogCacheService) {}

  async execute(): Promise<GetSeriesGenresUseCaseResponse> {
    const genres = await this.catalogCache.getSeriesGenres()
    return right({ genres })
  }
}
