...
interface GetTopRatedSeriesUseCaseRequest {
  page: number
}

type GetTopRatedSeriesUseCaseResponse = Either<
  never,
  {
...
export class GetTopRatedSeriesUseCase {
  ...
  async execute(
    request: GetTopRatedSeriesUseCaseRequest,
  ): Promise<GetTopRatedSeriesUseCaseResponse> {
    const data = await this.catalogCache.getTopRatedSeries(request.page)
